...
class Media:
    ...
    def download_image(self):
        url = 'https://picsum.photo/720/1280/?random'
    ...
    def process_image(self, text, author):
        ...
        image = Image.open("downloade_bg.png").filter(ImageFilter.GaussianBlur(10))
        image = ImageEnhance.Brightness(image)
        image.enhance(0.5).save('image.png')
        image = Image.open('image.png')
        draw = ImageDraw.Draw(image)
        font = ImageFont.truetype('XanhMono-Regular.ttf', size=29)
        ...
        if author is not None:
            ...
            font = ImageFont.truetype('XanhMono-Italic.ttf', size=29)
